...
session_start();
    ...
    function addOrder($userData, $orders_total, $orders_deposit, $orders_payment_method, $orders_code) {
        $data = array(
    ...
    function addOrderItems($orders_id , $category_id, $orders_item_checkin, $orders_item_checkout, $orders_item_price, $orders_item_quantity, $orders_item_count_people, $orders_item_description,$create_date,$update_date) {
        $data = array(
            'orders_id' => $orders_id,
            ...
            'orders_item_description' => $orders_item_description,
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST["checkout"])) {
            ...
            $dataOrders = addOrder($userData, $totalAmount, ($totalAmount/100*15), $orders_payment_method, $orders_code);
            $where_cart = 'users_id = ' . $userData["id"]; 
            delete_data('cart', $where_cart);
            for ($i = 0; $i < count($list_cart_items); $i++) {
            echo ($list_cart_items[$i]['id']);
                ...
                $orders_item_description = $list_cart_items[$i]['cart_item_description'];
                $create_date = date('Y-m-d');
                $create_date = date('Y-m-d');
                $addOrdersItems = addOrderItems($orders_id, $category_id, $orders_item_checkin, $orders_item_checkout, $orders_item_price, $orders_item_quantity, $orders_item_count_people, $orders_item_description,$create_date,$create_date);
